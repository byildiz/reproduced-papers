...
function ReprodForm() {
  ...
  function loadImage(event) {
    ...
    reader.onerror = () => {
      toast.error('Selected file could not read.')
      reader.abort()
    }
  ...
  function addTable() {
    ...
    if (keys.length > 0 && !tables[keys[keys.length - 1]].title) {
      toast.error('Fill in the added table(s) first')
      return
    }
    const key = uuidv4()
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          <label htmlFor="urlBlog" className="form-label">
        ...
        <div className="mb-3">
          <label className="col-form-label">Badges</label>
          {Object.keys(BADGES).map((key) => (
            <div key={key} className="form-check">
              ...
              <label className="form-check-label" htmlFor={key}>
                <span className={`badge text-bg-${BADGES[key].color}`}>
                  {BADGES[key].label}
                </span>
                <br />
                <small className="form-text text-muted">
